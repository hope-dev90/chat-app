...
export const uploadImageFile = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            file: { url: getUploadedUrl(req, 'images'), name: req.file.originalname, size: req.file.size, type: 'image' }
        });
    } catch (error) {
...
export const uploadRegularFile = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            file: { url: getUploadedUrl(req, 'files'), name: req.file.originalname, size: req.file.size, type: extension }
        });
    } catch (error) {
...
export const uploadAudioFile = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            file: { url: getUploadedUrl(req, 'audio'), name: req.file.originalname, size: req.file.size, type: 'audio' }
        });
    } catch (error) {
